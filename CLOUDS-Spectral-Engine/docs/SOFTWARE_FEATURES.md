...
| ID | Feature | Prio | Trace | Status |
|---|---|---|---|---|
| G-01 | Live HK display: temperatures, humidity, pressures, IMU, state, actuator status | P0 | §4.12.1 | ✔ actuator status is the `Membrane` and `Driving` rows (`ValveStatus` bits); a commanded drive is a 5 s pulse, so this is the only place ground sees it happen |
