...
| G-03 | Command console with arm/execute UI + full command set | P0 | §4.12.1 | ✔ plus an **Actuators** panel for the dispersion hardware - membrane duty + frequency with Drive/Stop; motor Speed slider with **Start / Stop** and **One pulse** (speed re-sent on slider release while running) - and `membrane <duty>` / `disperse [pulse|run|stop]` in the console monitor |
...
| G-07 | Link-quality panel (packet seq gaps, heartbeat RTT) | P2 | ops insight | ✔ plus an **Ethernet traffic indicator** in the sidebar header (2026-09-17, `clouds_ui/traffic.py`): three lanes - **Down** (UDP telemetry, counted where the datagram lands so a stream of CRC failures does not read as idle, amber over the 2 kbit/s budget), **Up** (TCP commands + PING heartbeat, ACK bytes counted back into the same lane so they never inflate the budget one) and **Bench** (the `--net` frame stream, ~50 kB/s, on its own row for the same reason) - each a blinking light, a 3 s-smoothed rate and the session total, with the far end named. It answers what the gap and age fields cannot: whether the cable is carrying *anything*. Red after 3 s of silence, grey where the session has no such link at all; outside the collapsible sections, because a fold that hides it turns "the link died" back into "the numbers stopped, why" |
